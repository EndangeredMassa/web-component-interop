...
function isCompilableExpression(expr$$1) {
    return (typeof expr$$1 === 'undefined' ? 'undefined' : _typeof(expr$$1)) === 'object' && expr$$1 !== null && typeof expr$$1.compile === 'function';
}
...
var OpcodeBuilder = function (_BasicOpcodeBuilder) {
    ...
    function OpcodeBuilder(env, meta) {
        ...
        var _this = possibleConstructorReturn(this, _BasicOpcodeBuilder.call(this, env, meta, program));
...
var Application = function () {
    ...
    Application.prototype.render = function render() {
        ...
        var doc = this.document; // TODO FixReification
        var appendTo = doc.body;
...
var ComponentDefinition$1 = function (_GlimmerComponentDefi) {
    ...
    function ComponentDefinition$$1(name, manager, template, componentFactory) {
        ...
        var _this = possibleConstructorReturn(this, _GlimmerComponentDefi.call(this, name, manager, componentFactory));
...
var LayoutCompiler = function () {
    function LayoutCompiler(name, template) {
        classCallCheck(this, LayoutCompiler);

        this.template = template;
        this.name = name;
    }

    LayoutCompiler.prototype.compile = function compile(builder) {
...
function setAttributes(glimmerComponent, webComponent) {
    var attrs = Array.prototype.slice.call(webComponent.attributes);
    glimmerComponent.htmlAttrs = attrs.reduce(function (newAttrs, node) {
        newAttrs[node.name] = parseAttribute(node.nodeValue);
        return newAttrs;
    }, {});
}

var AttrComponent = function (_Component) {
    inherits(AttrComponent, _Component);

    function AttrComponent() {
        classCallCheck(this, AttrComponent);
        return possibleConstructorReturn(this, _Component.apply(this, arguments));
    }

    // to catch new dom elements
    AttrComponent.prototype.didInsertElement = function didInsertElement() {
        var _this2 = this;

        var webComponent = this.element.parentNode.host;
        // glimmer's web component's attribute changed callback
        // will invoke this for us, if it exists
        webComponent.onAttributeChangedCallback = function (attr, newValue) {
            var attrs = Array.prototype.slice.call(webComponent.attributes);
            setAttributes(_this2, webComponent);
        };
        setAttributes(this, webComponent);
    };

    AttrComponent.prototype.dispatchEvent = function dispatchEvent(name, args) {
        var webComponent = this.element.parentNode.host;
...
__decorate([tracked], AttrComponent.prototype, "htmlAttrs", void 0);

var SelectList = function (_AttrComponent) {
    inherits(SelectList, _AttrComponent);

    function SelectList() {
        classCallCheck(this, SelectList);
        return possibleConstructorReturn(this, _AttrComponent.apply(this, arguments));
    }

    SelectList.prototype.selectItem = function selectItem(itemValue) {
...
function glimmerElementFactory(app, componentName, trackedAttributes) {
...
function initializeCustomElements(app, customElementDefinitions) {
    customElementDefinitions.forEach(function (definition) {
        if ((typeof definition === 'undefined' ? 'undefined' : _typeof(definition)) === 'object') {
            initializeCustomElement(app, definition.name, definition.trackedAttributes);
        } else {
            initializeCustomElement(app, definition);
        }
    });
}
function initializeCustomElement(app, name, trackedAttributes) {
    window.customElements.define(name, glimmerElementFactory(app, name, trackedAttributes));
}
